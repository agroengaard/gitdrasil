...
Foil::Foil(std::string name, double chord_length, double angle_of_attack) {
    this->name             = name;
...
NACA::NACA(std::string name, double chord_length, double angle_of_attack)
    : Foil(name, chord_length, angle_of_attack) {}

void NACA::calculate_thickness_distribution() {
...
NACA4DIGIT::NACA4DIGIT(std::string name, double chord_length, double angle_of_attack)
    : NACA(name, chord_length, angle_of_attack) {
    calculate_camber_line();
    calculate_thickness_distribution();
    calculate_ordinates();
}

void NACA4DIGIT::calculate_camber_line() {
    ...
    if (M == 0) {
        ...
        VectorXd yc1     = (M/(P*P)) * ((2.0*P*x1.array()) - x1.array().square()).matrix();
        VectorXd yc2     = ((M / ((1.0-P)*(1.0-P))) * (1.0 - (2.0*P) + (2.0*P*x2.array()) - x2.array().square())).matrix();
        VectorXd dyc_dx1 = (((2.0*M) / (P*P)) * (P - x1.array())).matrix();
...
NACA5DIGIT::NACA5DIGIT(std::string name, double chord_length, double angle_of_attack)
    : NACA(name, chord_length, angle_of_attack) {
    calculate_camber_line();
    calculate_thickness_distribution();
    calculate_ordinates();
...
void NACA5DIGIT::calculate_camber_line() {
...
Biconvex::Biconvex(std::string name, double chord_length, double angle_of_attack)
    : Foil(name, chord_length, angle_of_attack) {
    calculate_pts();
};

void Biconvex::calculate_pts() {

    double tau = 0.05;

    double n_ini_pts = int(num_pts/2)+1;

    double Ox  = 0.5;
    double r   = (tau/2)+ (1/(8*tau));
    double Oy  = -1*(r-tau);
    double x_i = Ox-r;
    double y_i = Oy-r;
    double startangle = std::asin(Oy/r);
    double pi = 2*acos(0.0);
    double endangle   = pi - startangle;

    VectorXd t_pts = VectorXd::LinSpaced(n_ini_pts, startangle, endangle);
    VectorXd x_pts = r* t_pts.array().cos() + Ox;
    VectorXd y_pts = r* t_pts.array().sin() + Oy;

    // Build closed contour: x forward, then x reversed (excluding endpoints), mirrored y
    VectorXd x_flip = x_pts.reverse();
    VectorXd y_flip = (-y_pts).reverse();

    VectorXd x_mid = x_flip.segment(1, x_flip.size() - 2);
    VectorXd y_mid = y_flip.segment(1, y_flip.size() - 2);

    Pts_x.resize(x_pts.size() + x_mid.size());
    Pts_x << x_pts, x_mid;

    Pts_y.resize(y_pts.size() + y_mid.size());
    Pts_y << y_pts, y_mid;
